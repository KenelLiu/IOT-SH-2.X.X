...
import iot.sh.exception.HttpException;
import iot.sh.util.system.SystemPropertyUtil;
...
public class HttpPool45 extends HttpPool{
	...
	static {
		try{
			int socketTimeout=(SystemPropertyUtil.getInt(ToolsConstants.HTTP.SocketTimeout_MS, 30))*1000;//默认 30秒 （单位毫秒）
			int connectTimeout=(SystemPropertyUtil.getInt(ToolsConstants.HTTP.ConnectionTimeout_MS, 30))*1000;//默认 30秒 （单位毫秒）
			int requestTimeout=(SystemPropertyUtil.getInt(ToolsConstants.HTTP.RequestTimeout_MS, 30))*1000;//默认 30秒 （单位毫秒）
			
			int defaultMaxPerRoute= SystemPropertyUtil.getInt(ToolsConstants.HTTP.DefaultMaxPerRoute, 100);//默认100
			int maxTotal=SystemPropertyUtil.getInt(ToolsConstants.HTTP.MaxTotal, 600);//默认600
			int retryTime=SystemPropertyUtil.getInt(ToolsConstants.HTTP.RetryTime, 1);//默认1
			String keyStoreFile=SystemPropertyUtil.get(ToolsConstants.HTTP.SelfKeyStoreFile, null);
			...
			if(keyStoreFile==null){
				...
				String selfSplitChar=SystemPropertyUtil.get(ToolsConstants.HTTP.SelfSplitChar, ",");
				String keystorePassword=SystemPropertyUtil.get(ToolsConstants.HTTP.SelfKeyStorePassword, "123456");//访问keystore的密码
				...
				String ignoreVerifyHost=SystemPropertyUtil.get(ToolsConstants.HTTP.SelfIgnoreVerifyHost, "");
