...
import iot.sh.model.FileHttpResponse;
...
public class HttpClient {
	...
	protected  <T>T handleResponse(ResponseHandler<T> handler) {
		...
		try {
			...
			throw new iot.sh.exception.HttpException("httpClient send request error:"+ e.getMessage());
		}finally{			
			//释放资源
	...
	private void setRequestParam() {
		...
		try {
			if (this.url == null || this.url.trim().length()==0) {
				throw new iot.sh.exception.HttpException("http url is null");
			}				
